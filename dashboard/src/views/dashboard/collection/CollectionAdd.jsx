...
import {messageClear,get_all_payment_mode,get_all_customers,get_banks,get_agents,collection_add,get_all_collection} from '../../../store/reducers/collectionReducer'
...
const CollectionAdd = () => {
  ...
  const {loader,successMessage,errorMessage,customers,banks,agents,payModes,collections,totalCollection} = useSelector(state=>state.collection)
...
const collectionAdd= (e)=>{
    e.preventDefault()
    dispatch(collection_add({
      state:state,
      chequeDate,
      custId,
      payModeId,
...
useEffect(()=>{
  ...
  dispatch(get_banks())
  dispatch(get_agents())
  dispatch(get_all_collection({searchValue}))
},[searchValue])
  ...
  function AllRows({ index, style }) {
    return <div style={style} className='flex text-[11px] mt-2'>
      ...
      <div className='w-[15%] p-1 whitespace-nowrap'>{`${collections[index]?.customer[0]?.cus_company}`}</div>
      <div className='w-[12%] p-1 whitespace-nowrap'>{`${collections[index]?.payMode[0]?.pay_mode}`}</div>
      <div className='w-[12%] p-1 whitespace-nowrap'>{`${collections[index]?.cheque_no}`}</div>
      <div className='w-[12%] p-1 whitespace-nowrap'>{moment(`${collections[index]?.cheque_date}`).format('LL')}</div>
      <div className='w-[10%] p-1 whitespace-nowrap'>{`${collections[index]?.bank[0].bank_short_name}`}</div>
      <div className='w-[12%] p-1 whitespace-nowrap'>{moment(`${collections[index]?.pay_date}`).format('LL')}</div>
      <div className='w-[12%] p-1 whitespace-nowrap'>{`${collections[index]?.pay_amount.toFixed(2)}`}</div>

      <div className="w-[10%] flex justify-center items-center gap-1">
        <Link to={`/admin/dashboard/edit-collection/${collections[index]?._id}`} className="p-[4px] mt-[-7px] rounded bg-yellow-500 hover:shadow-lg shadow-yellow-500/50">
          <FaEdit />
        </Link>
      ...
      <div className="flex flex-wrap w-[98%] mx-auto">
          <div className="w-full">
            ...
            <div className="flex justify-between items-center bg-[#283046] py-1 md:px-[10px] text-white">
                <div className="pl-2 font-bold text-2xl">Collection Create</div>
            </div>
              <hr/>
              <form onSubmit={collectionAdd}>
