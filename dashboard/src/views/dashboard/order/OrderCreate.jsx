...
import {messageClear,last_order_number_show,get_order_customers,get_order_products,get_order_product_info,order_create,show_order_details,order_details_item_delete} from '../../../store/reducers/orderReducer'
...
const OrderCreate = () => {
  ...
  const {loader,successMessage,errorMessage,products,customers,productInfo,lastOrderNumberShow,orderProductDetails,productTotalPerOrder} = useSelector(state=>state.order)
...
useEffect(()=>{
  ...
  dispatch(last_order_number_show())
},[])
// useEffect(()=>{
      ...
      <div className="flex flex-wrap w-[99%]">
          <div className="w-full pl-2 mt-[-25px]">
            ...
            <div className="flex justify-between items-center bg-[#283046] py-2 md:px-[2px] text-white">
                <div className="pl-2 font-bold text-2xl">Order Create</div>
            ...
            <hr className='w-[100%] border-2 border-slate-500 mx-auto'/>
              ...
              <div className='flex flex-col mb-1 md:flex-row gap-1 w-full text-[#d0d2d6] mt-2'>
                <div className='flex flex-col w-[25%] gap-1'>
                    ...
                    <input className='bg-[#FFFFFF]  px-2 py-1 border border-slate-700 rounded-[3px] outline-none focus:border-indigo-500 text-[#2A2D2E] text-xs' onChange={(e)=>setOrderNumber(e.target.value)} value={orderNumber} type='text' id='order_number' name='order_number' required/>
                </div>
                ...
                <div className='flex flex-col w-[25%] gap-1'>
                <label htmlFor='invoice_due_date' className="text-xs text-white">Delivery Date:</label>   
                  <div className="bg-[#FFFFFF] text-black text-xs px-[8px] py-[5px] rounded-[3px]">
                    <DatePicker                        
...
<button  onClick={()=>navigate('/admin/dashboard/order-list')} className="bg-[#4ac25e] ml-3 px-3 py-1 rounded-sm font-semibold">Save & Go To List</button>
          </div>
          </div>       
      </div>
    </div>
