...
import {messageClear,get_expenses_report_data} from "../../../store/reducers/reportReducer";
...
const ExpensesReport = () => {
  ...
  const ref = useRef()
  const { expensesData,errorMessage,successMessage} = useSelector(state => state.report);
...
const paymentReportData = (e)=>{
      e.preventDefault()
      dispatch(get_expenses_report_data({
        startDate,
        endDate
      }))
}
...
function AllRows({ index, style }) {
  return <div style={style} className='flex text-xs mt-2'>
    ...
    <div className='w-[20%] text-xs p-1 whitespace-nowrap'>{`${expensesData[index]?.expense_name}`}</div>
    <div className='w-[20%] text-xs p-1 whitespace-nowrap'>{`${expensesData[index]?.expcat[0]?.expense_cat_name}`}</div>
    <div className='w-[20%] text-xs p-1 whitespace-nowrap'>{moment(`${expensesData[index]?.expense_date}`).format('LL')}</div>
    <div className='w-[20%] text-xs p-1 text-left whitespace-nowrap'>{`${expensesData[index]?.customer[0]?.cus_company}`}</div>
    <div className='w-[15%] text-xs p-1 whitespace-nowrap'>{`${expensesData[index]?.expense_amount}`}</div>
  </div>
} 
useEffect(()=>{
  if(successMessage){
            ...
            <div className="bg-[#fefeff] text-black text-sm w-[92%] py-[6px] mx-auto text-center">
              ...
              <div className='w-ful overflow-x-auto'>
                    <div className='flex uppercase font-bold text-xs min-w-[340px] text-black no-scrollbars'>
                        ...
                        <div className='w-[20%] text-left text-xs p-2'>Expense Name</div>
                        <div className='w-[20%] text-xs p-2'>Expense Category</div>
                        <div className='w-[20%] text-xs p-2'>Date</div>
                        <div className='w-[20%] text-xs p-2'>Customer</div>
                        <div className='w-[15%] text-xs p-2'>Amount</div>
                    </div>
                    {
                        <List 
