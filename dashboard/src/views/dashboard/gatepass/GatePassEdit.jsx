...
import {messageClear,get_agents,get_customers,get_products,get_gp_types,get_terms,gate_pass_add,last_gate_pass_number_show,get_product_info,show_gate_pass_product_details,g_pass_product_details_item_delete,gp_get_by_id,gp_info_update} from '../../../store/reducers/gatePassReducer'
...
const GatePassEdit = () => {
  ...
  const {loader,successMessage,errorMessage,agents,customers,terms,gpTypes,products,productInfo,gatePassProductDetails,productTotalPerGatePass,editGpInfo} = useSelector(state=>state.gatePass)
...
const gpUpdateInfo = (e)=>{
  ...
  dispatch(gp_info_update(obj))
}
useEffect(()=>{
  if(editGpInfo.length>0){
    setGpDate(editGpInfo[0]?.gp_date)
    setGpReturnDate(editGpInfo[0]?.gp_return_date)
    setGpNumber(editGpInfo[0]?.gp_number)
    setClientNote(editGpInfo[0]?.client_note)
    setSaleAgentId(editGpInfo[0]?.agent[0]?._id)
    setGpTypeId(editGpInfo[0]?.type[0]?._id)
    setGpTermId(editGpInfo[0]?.term[0]?._id)
    setCusId(editGpInfo[0]?.customer[0]?._id)
  }
 },[editGpInfo]) 
...
useEffect(()=>{
  if(gatePassId){
    dispatch(gp_get_by_id(gatePassId))
  }
},[gatePassId])
      ...
      <div className="flex flex-wrap w-[98%] mx-auto">
          <div className="w-full">
            ...
            <div className="flex justify-between items-center bg-[#283046] py-1 md:px-[10px] text-white">
                <div className="pl-2 font-bold text-3xl">Gate Pass Edit</div>
              ...
              <form onSubmit={GatePassAdd}>
                    <div className='flex flex-col mb-1 md:flex-row gap-2 w-full px-3 text-[#d0d2d6] mt-1'>
                            ...
                            <div className='flex flex-col w-[20%] gap-1'>  
                              <label htmlFor='gp_type_id' className="text-xs text-white">Gate Pass Type:</label>                              
                                  <div style={{width:180,color:'black'}}>                 
                                        ...
                                        value={gpTypeOptions.find(option=>option.value === gpTypeId)}
                        ...
                        <div className='flex flex-col w-[16%] gap-1 text-xs'>
                        <label htmlFor="gp_number">Gp Number</label>
                        ...
                        <label htmlFor='sale_agent_id' className="text-xs text-white">Sale Agent:</label>                              
                            <div style={{width:180,color:'black'}}>                 
                                ...
                                value={agentOptions.find(option=>option.value === saleAgentId)}
                     ...
                     <div className='flex flex-col w-[20%] gap-1'>  
                        <label htmlFor='term_id' className="text-xs text-white">Term:</label>                              
                            <div style={{width:180,color:'black'}}>                 
                                <Select
                                options={termOptions}
                                value={termOptions.find(option=>option.value === gpTermId)}
                    ...
                    <div className='flex flex-col w-[23%] gap-1'>
                          ...
                          <label htmlFor='return_date' className="text-xs text-white">Return Date:</label>   
                            <div className="bg-[#FFFFFF] text-black px-[8px] py-[4px] rounded-[3px]">
                              <DatePicker
                                  selected={new Date(gpReturnDate)}
                    ...
                    <div className='flex flex-col w-[44%] gap-1 text-xs'>
                    ...
                    <div className='flex flex-col mt-[22px] w-[10%] gap-[1px]'>
                        <button type="submit" onClick={gpUpdateInfo} className='bg-blue-500 text-xs px-2 py-[8px] mb-3 text-white hover:shadow-blue-500/50 hover:shadow-md font-bold rounded-md'> Update </button>
                    </div>
                    </div>
            ...
            <div className='relative overflow-x-auto bg-[#283046] md:min-h-[325px]'>
