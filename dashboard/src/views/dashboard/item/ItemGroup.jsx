...
import {messageClear,item_group_add,get_item_group,delete_item_group} from '../../../store/reducers/itemGroupReducer'
...
const ItemGroup = () => {
  ...
  const {loader,successMessage,errorMessage,itemGroups,totalItemGroup} = useSelector(state=>state.itemGroup)
...
const inputHandle = (e)=>{
  ...
  const add_item_group= (e)=>{
      e.preventDefault()
      dispatch(item_group_add({
        state:state,
...
const AllRows = ({index,style})=>{
  return <div  style={style} className='flex text-xs'>
      <div className='w-[10%] p-2 border border-[#4E5B7C] whitespace-nowrap'>{index+1}</div>
      <div className='w-[35%] p-2 border border-[#4E5B7C]  whitespace-nowrap'>{`${itemGroups[index].item_group_name}`}</div>
      <div className='w-[35%] border border-[#4E5B7C]  p-2 '>{`${itemGroups[index].item_group_desc}`}</div>
      <div className="w-[20%] border border-[#4E5B7C]  flex justify-center items-center gap-1">
      <Link to= {`/admin/dashboard/edit-item-group/${itemGroups[index]._id}`} className="p-[5px] rounded bg-yellow-500 hover:shadow-lg shadow-yellow-500/50">
        <FaEdit />
      </Link>
        <button onClick={()=>dispatch(delete_item_group(`${itemGroups[index]._id}`))} className="p-[5px] rounded bg-red-600 hover:shadow-lg shadow-red-600/50">
          <FaTrash />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="px-2 py-5 md:px-[2px]">
      <div className='lg:hidden flex justify-between items-center mb-5 p-3 bg-[#283046] rounded-sm'>
           <h1 className='text-[#d0d2d6] font-semibold text-lg'>Items Group</h1> 
          ...
          <div className="w-full pl-5">
            <div className="bg-[#283046] text-[#d0d2d6] height-screen lg:height-auto rounded-l-lg px-3 py-2">
              ...
              <form onSubmit={add_item_group}>
                ...
                <div className='mt-4'>
                <button disabled={loader?true:false} type="submit" className='bg-blue-500 w-full px-7 py-2 rounded-md mb-3 text-white hover:shadow-blue-500/50 hover:shadow-md font-bold'>
              {
                loader? <PropagateLoader color='#fff' cssOverride={cssOverride}/>: 'Add Group'
              }
            ...
            <div className="w-full p-4 bg-[#283046] rounded-md mt-6">
              ...
              <div className='w-full overflow-x-auto'>
                    <div className='flex bg-[#007DC3] uppercase font-bold text-xs min-w-[340px] text-white'>
                        ...
                        <div className='w-[35%] p-2'>Item Group Name</div>
                        <div className='w-[35%] p-2'>Group Description</div>
