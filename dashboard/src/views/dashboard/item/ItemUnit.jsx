...
import {messageClear,unit_add,get_item_unit,delete_item_unit} from '../../../store/reducers/unitReducer'
...
const ItemUnit = () => {
  ...
  const {loader,successMessage,errorMessage,itemUnits,totalItemUnit} = useSelector(state=>state.unit)
...
const AllRows = ({index,style})=>{
  return <div style={style} className='flex text-sm'>
      ...
      <div className='w-[35%] p-2 whitespace-nowrap'>{`${itemUnits[index].unit_name}`}</div>
      <div className='w-[35%] p-2 whitespace-nowrap'>{`${itemUnits[index].unit_desc}`}</div>
      <div className="w-[20%] flex justify-center items-center gap-1">
      <Link to= {`/admin/dashboard/edit-item-unit/${itemUnits[index]._id}`} className="p-[5px] rounded bg-yellow-500 hover:shadow-lg shadow-yellow-500/50">
        <FaEdit />
      </Link>
        <button onClick = {()=>dispatch(delete_item_unit(`${itemUnits[index]._id}`))} className="p-[5px] rounded bg-red-600 hover:shadow-lg shadow-red-600/50">
          <FaTrash />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="px-2 py-5 md:px-[2px]">
      <div className='lg:hidden flex justify-between items-center mb-5 p-3 bg-[#283046] rounded-sm'>
           <h1 className='text-[#d0d2d6] font-semibold text-lg'>Item Unit</h1> 
        ...
        >
          <div className="w-full pl-5">
            <div className="bg-[#283046] text-[#d0d2d6] height-screen lg:height-auto rounded-l-lg px-3 py-2">
              ...
              <form onSubmit={add_item_unit}>
                <div className="flex flex-col w-full gap-1 mb-3">
                  <label htmlFor="unit_name">Item Unit Name</label>
                  <input
                   value={state.unit_name}  onChange={inputHandle} className="bg-[#283046] border border-slate-700 px-4 py-1 focus:border-indigo-500 outline-none text-[#d0d2d6] rounded-md"
                    ...
                    id="unit_name"
                    name="unit_name"
                    placeholder="Enter item unit" required
                  />
                </div>
                <div className="flex flex-col w-full gap-1 mb-3">
                <div className='flex flex-col w-full gap-1 text-[#d0d2d6] mb-4'>
                        <label htmlFor='unit_desc'>Description:</label>
                        <textarea  rows={10} onChange={inputHandle} value={state.unit_desc} name='unit_desc' id='unit_desc' className='bg-[#283046] px-4 py-1 border border-slate-700 rounded-sm outline-none focus:border-indigo-500 text-[#d0d2d6]' placeholder='Write here..'></textarea>
                    </div>
                </div>
              ...
              {
                loader? <PropagateLoader color='#fff' cssOverride={cssOverride}/>: 'Add Unit'
              }
            ...
            <div className="w-full p-4 bg-[#283046] rounded-md mt-6">
              ...
              <div className='w-full overflow-x-auto'>
                    <div className='flex bg-[#007DC3] uppercase font-bold text-xs min-w-[340px] text-white'>
                        ...
                        <div className='w-[35%] p-2'>Unit Name</div>
                        <div className='w-[35%] p-2'>Unit Description</div>
