...
import {messageClear,get_payment_report_data} from "../../../store/reducers/reportReducer";
...
const PaymentReport = () => {
  ...
  const ref = useRef()
  const { paymentData,errorMessage,successMessage} = useSelector(state => state.report);
...
const paymentReportData = (e)=>{
      e.preventDefault()
      dispatch(get_payment_report_data({
        startDate,
        endDate
      }))
}
...
function AllRows({ index, style }) {
  return <div style={style} className='flex text-xs mt-2'>
    ...
    <div className='w-[25%] text-xs p-1 text-left whitespace-nowrap'>{`${paymentData[index]?.vendor[0]?.vendor_company}`}</div>
    <div className='w-[15%] text-xs p-1 whitespace-nowrap'>{moment(`${paymentData[index]?.pay_date}`).format('LL')}</div>
    <div className='w-[15%] text-xs p-1 whitespace-nowrap'>{`${paymentData[index]?.pmode[0]?.pay_mode}`}</div>
    <div className='w-[10%] text-xs p-1 whitespace-nowrap'>{`${paymentData[index]?.bank[0]?.bank_short_name}`}</div>
    <div className='w-[15%] text-xs p-1 whitespace-nowrap'>{`${paymentData[index]?.cheque_no}`}</div>
    <div className='w-[15%] text-xs p-1 whitespace-nowrap'>{`${paymentData[index]?.pay_amount}`}</div>
  </div>
} 
useEffect(()=>{
  if(successMessage){
            ...
            <div className="bg-[#fefeff] text-black text-sm w-[92%] py-[6px] mx-auto text-center">
              ...
              <div className='w-ful overflow-x-auto'>
                    <div className='flex uppercase font-bold text-xs min-w-[340px] text-black no-scrollbars'>
                        ...
                        <div className='w-[25%] text-left text-xs p-2'>Vendor</div>
